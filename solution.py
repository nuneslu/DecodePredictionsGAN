...
def train_gan(epochs, lr, dataset_dir):
    ...
    train_loader = torch.utils.data.DataLoader(Emnist(root_dir='./data/emnist', transforms=transform), batch_size=8)
    print('here')
...
def save_net(gen, disc):
    torch.save({'model_state_dict': gen.state_dict()}, 'generator_emnist.pt')
    torch.save({'model_state_dict': disc.state_dict()}, 'discriminator_emnist.pt')

def classify_images():
    preds = []
    model = getattr(resnet, f"cifar100_resnet20")()

    checkpoint = torch.load('cifar_model/cifar100_resnet20-23dac2f1.pt')
    model.load_state_dict(checkpoint)
    model.eval()

    transform = transforms.Compose([transforms.ToTensor()])
    dataset = torchvision.datasets.CIFAR100("./data/cifar", train=True, transform=transform, download=True)
    ...
    for filename in sorted(files):
        img = transform(PIL.Image.open(f'./images/infer/{filename}'))
...
def generate_message_from_encode(encode):
    ...
    print(encode_pred)
    gen = generator(input_size=14, n_class=28*28)
    checkpoint = torch.load('gan_pretrained/generator_emnist.pt', map_location=torch.device('cpu'))
